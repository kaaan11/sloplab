...
def study(config: str, out: str) -> None:
    ...
    # bytes, coverage, and definition version; historical analysis.json is
    ...
    write_versioned_analysis(
        ...
        bundles={n: _metrics_to_dict(b) for n, b in bundles.items()},
        coverage={
            n: {"scored": len(by_evaluator[n]), "failed": failed_by_evaluator.get(n, 0)}
            for n in names
        },
    )
