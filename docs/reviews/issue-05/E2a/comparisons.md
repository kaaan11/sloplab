# E2a — E1e-review referanslı karşılaştırma

Yeni koşu: `/tmp/sloplab-e2a-u0PKCr/run` (E2a koduyla, 297 vaka, 2 evaluator).
...
- 7 semantic hash eşit: generation, evaluation, analysis, settings, inputs,
  selection, execution.
- `locations` eşit (çözümlenmiş suite/corpus yolları aynı).
- `metadata` eşit: head `9960f4f…`, dirty `true` (tam kod kimliği diye
  sunulmaz), CPython 3.13.15, sloplab 0.2.2, lock `4a4969aa…`.
- Kayıt satırı: 594 (evaluator başına 297; canonical 120 + mutated 474).

Pilot failure çıktıları bilinçli protokol değişimidir; eski failure bayt eşitliği
aranmadı (deterministik kayıtlarda failure placeholder yoktu/yok).
