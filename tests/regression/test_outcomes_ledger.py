...
def test_legacy_placeholder_rejected_unexpected_propagates_config_clean(
    ...
    # E3b-r1 protocol: the interrupted publish leaves an in-progress marker
    # (no completion, no records); every reader refuses it.
    from sloplab.experiments.bundle import classify_bundle

    assert (tmp_path / "o-boom" / "publish-in-progress.json").is_file()
    assert not (tmp_path / "o-boom" / "completion.json").exists()
    assert not (tmp_path / "o-boom" / "records.jsonl").exists()
    assert classify_bundle(tmp_path / "o-boom") == "incomplete"
